...
local function theme_components()
  local function color(group, attr, fallback)
    local value = vim_api.nvim_get_hl(0, { name = group, link = false })[attr]
    return value and string.format("#%06x", value) or fallback
  end
  local background = color("Normal", "bg", "#1e1e2e")
  local colors = {
    bg = background,
    bg_dark = background,
    gray2 = color("CursorLine", "bg", background),
    red = color("DiagnosticError", "fg", "#f38ba8"),
    bright_red = color("DiagnosticError", "fg", "#f38ba8"),
    yellow = color("DiagnosticWarn", "fg", "#f9e2af"),
    blue = color("Function", "fg", "#89b4fa"),
    purple = color("Statement", "fg", "#cba6f7"),
    green = color("String", "fg", "#a6e3a1"),
    cyan = color("DiagnosticHint", "fg", "#94e2d5"),
  }
  ...
  return colors, modecolor, scrollbar
end
...
local function get_lsp_names()
  ...
  local linters = lint and lint.linters_by_ft[vim_bo.filetype] or {}
  for _, name in ipairs(type(linters) == "string" and { linters } or linters) do
    add(name)
  end
  table.sort(names)
  return #names > 0 and table.concat(names, ", ") or "  No servers"
end
...
return {
  {
    ...
    config = function()
      local function setup()
        local colors, modecolor, scrollbar = theme_components()
        ...
        local filename = {
          function()
            ...
            if vim.o.columns > 100 and #cwd > 0 and #ftype > 0 then
              if vim.startswith(fname, cwd .. "/") then
                local relative_path = fname:sub(#cwd + 2)
                local shortened_path = vim.fn.pathshorten(relative_path)
        ...
        local modes = {
          ...
          color = function()
            ...
            return { bg = mode_color[vim_fn.mode()] or colors.red, fg = colors.bg_dark, gui = "bold" }
          end,
          separator = { left = "", right = "" },
        }
      ...
      setup()
      vim_api.nvim_create_autocmd("ColorScheme", {
        group = vim_api.nvim_create_augroup("config_lualine_theme", { clear = true }),
